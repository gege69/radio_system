...
//				.and()
	...
	@Configuration
	@Order(3)  
    public static class FormLoginWebPlayerSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
...
//				.accessDeniedHandler( myAccessDeniedHandler )
			...
			// in the order they are configured below. So /** (anyRequest()) should always be at the bottom of the list.
			.authorizeRequests()
				.antMatchers( "/loginplayer" ).permitAll()
				.antMatchers( "/player**" ).hasAuthority( "PLAYER" )
				.anyRequest().authenticated()
				.and()
//			.requiresChannel()
...
//				.and()
			...
			.formLogin()
				.loginPage( "/loginplayer" )
				.defaultSuccessUrl("/player", true)
				.failureUrl( "/loginplayer?err=1" )
				.successHandler( simpleAuthenticationHandler )
				.usernameParameter( "username" )
				.passwordParameter( "password" )
				.and()
			...
			.logout()
				.logoutRequestMatcher( new AntPathRequestMatcher( "/logout" ) )
				.logoutSuccessUrl( "/loginplayer?out=1" )
				.deleteCookies( "JSESSIONID" )
				.invalidateHttpSession( true )
				.permitAll()
				.and()
			...
			// compulsory but can add some extra security to your application.
			.sessionManagement()
				.invalidSessionUrl( "/loginplayer?time=1" )
				.maximumSessions( 1 );
		}
	} 
